plot_modified <- function (x, x2 = NULL, subject = 1, outcomes = 1,
  ...
  if (!is.null(pred_Long) && !is.null(pred_Event)) {
    if (n_outcomes == 1) {
      ...
      # n_outcomes == 3
      op <- par(mfrow = c(length(outcomes), 1), 
                oma = c(4,4,3,4), 
                mar = c(0, 0, 0, 0),
                mgp = c(2, 0.4, 0), 
                tcl = -0.3, 
                bg = bg)
      pp <- par("usr")[3] + pos_ylab_long * diff(par("usr")[3:4])
      for(w in 1:length(outcomes)){
        plot_long_i(outcomes[w], box = FALSE, cex_axis = cex_axis)
        mtext(ylab_long[outcomes[w]], side = 2, line = 1.5, #at = pp[w],
              cex = cex_ylab_long * 0.66, col = col_axis,srt=0)
      }
      # plot_long_i(outcomes[2L], box = FALSE, cex_axis = cex_axis)
      # mtext(ylab_long[outcomes[2L]], 2, 1.5, at = pp[2],
      #       cex = cex_ylab_long * 0.66, col = col_axis)
